...
const totalSteps = isEditing.value ? 2 : 3

const steps = computed(() => {
  if (isEditing.value) {
    return [
      { number: 1, label: '¿Qué?' },
      { number: 2, label: '¿Cuándo?' },
    ]
  }
  return [
    { number: 1, label: '¿Qué?' },
    { number: 2, label: '¿Cuándo?' },
    { number: 3, label: '¿Cuándo próxima?' },
  ]
})
...
const canAdvance = computed(() => {
  ...
  if (currentStep.value === 3) {
    if (!wantsNext.value) return true
...
function formatCategoryLabel(cat: HealthCatalogCategoryType): string {
    ...
    <div class="modal-backdrop" @click.self="emit('close')">
      <div class="modal-container">
        <div class="modal-header">
          <h2>{{ isEditing ? `Editar ${formatCategoryLabel(category)}` : `Registrar ${formatCategoryLabel(category)}` }}</h2>
          <button class="btn-close" @click="emit('close')">
            <IconX :size="18" :stroke-width="2" />
          </button>
        </div>

        <div v-if="!isEditing" class="stepper-bar">
          <div
            v-for="step in steps"
            :key="step.number"
            class="step-item"
        ...
        <div class="modal-body">
          <div v-if="currentStep === 1" class="step-content">
            <p class="step-instruction">
              Seleccioná el {{ formatCategoryLabel(category) }}
            </p>
            ...
            <div class="catalog-search-box">
              ...
              <input
                ...
                :placeholder="`Buscar ${formatCategoryLabel(category)}...`"
              />
            </div>
            ...
            <div v-if="selectedCatalogId === 'custom'" class="custom-input">
              ...
              <input
                v-model="customName"
                class="catalog-search-input"
                placeholder="Ej: Vacuna X, Pastilla Y..."
              />
            </div>
          </div>
          ...
          <div v-if="currentStep === 3 && !isEditing" class="step-content">
            <p class="step-instruction">
              ¿Querés programar la próxima aplicación?
            </p>
                ...
                <div class="suggestion-header">
                  <p class="suggestion-text">
                    ...
                    <template v-if="selectedCatalog?.frequency_months">
                      <br>Este {{ formatCategoryLabel(category) }} se aplica cada <strong>{{ selectedCatalog.frequency_months }} meses</strong>.
                    </template>
                  </p>
